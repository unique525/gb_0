...
$(function() {
    ...
    btnModify.click(function(event) {
        event.preventDefault();
        var product_param_type_classId = $(this).attr('idvalue');
        var pageIndex = Request["p"]==null?1:Request["p"];
        pageIndex =  parseInt(pageIndex);
        if (pageIndex <= 0) {
            pageIndex = 1;
        }
        var url='/default.php?secu=manage&mod=product_param_type_class&m=modify&product_param_type_class_id=' + product_param_type_classId + '&p=' + pageIndex;
        $("#dialog_frame").attr("src",url);
        $("#dialog_resultbox").dialog({
    ...
    $(".btn_open_product_param_type_list").click(function(event) {
        event.preventDefault();
        var product_param_type_classId=$(this).attr('idvalue');
        var product_param_type_classTitle=$(this).attr('title');
        parent.G_TabUrl = '/default.php?secu=manage&mod=product_param_type&m=list&product_param_type_class_id=' + product_param_type_classId;
        parent.G_TabTitle = product_param_type_classTitle + '-编辑产品参数类型';
        parent.addTab();
    });
