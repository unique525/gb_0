    ...
    <script type="text/javascript">
        ...
        $(function(){
            ...
            $("#btn_create").click(function (event) {
                ...
                window.location.href = '/default.php?secu=manage&mod=team&m=create&tab_index='+ parent.G_TabIndex +'&site_id={SiteId}';
            });
            //编辑
            $(".btn_edit").click(function (event) {
                event.preventDefault();
                var teamId = parseInt($(this).attr("idvalue"));
                window.location.href = '/default.php?secu=manage&mod=team&m=modify&tab_index='+ parent.G_TabIndex +'&team_id='+teamId;
            });
            //导入
            $("#btn_import").click(function (event){
                event.preventDefault();
                window.location.href = '/default.php?secu=manage&mod=team&m=import&tab_index='+ parent.G_TabIndex +'&site_id={SiteId}';
            });
...
<div class="div_list">
    <table width="100%" cellpadding="0" cellspacing="0">
        <tr>
            <td id="td_main_btn" width="260">
                <input id="btn_create" class="btn2" value="新增球队" title="" type="button"/>
    ...
    <table width="100%" class="grid" cellpadding="0" cellspacing="0" id="left_tree">
        ...
        <icms id="team_list" type="list">
            <item><![CDATA[
                <tr class="grid_item" id="{f_TeamId}">
                    <td class="spe_line" style="text-align:center">{f_TeamId}</td>
                    <td class="spe_line2" style="text-align:center;"><img style="cursor: pointer"
                                                                          class="btn_edit"
                                                                          src="/system_template/default/images/manage/edit.gif"
                                                                          alt="编辑" title="{f_TeamId}"
                                                                          idvalue="{f_TeamId}"
                                                                          onclick="Modify('{f_TeamId}')"/></td>                    <td class="spe_line2" style="width:40px;text-align:center;"><span id="span_state_{f_TeamId}" class="span_state" idvalue="{f_TeamId}">{f_State}</span></td>
                    <td class="spe_line2" style="width:60px;text-align:center;">
                        <img class="img_open" idvalue="{f_TeamId}" src="/system_template/{template_name}/images/manage/start.jpg" style="cursor:pointer" onclick="modifyState({f_TeamId},0)"/>
                        &nbsp;
                        <img class="img_close" idvalue="{f_TeamId}" src="/system_template/{template_name}/images/manage/stop.jpg" style="cursor:pointer" onclick="modifyState({f_TeamId},100)"/>
                    </td>
                    <td class="spe_line">{f_TeamName}</td>

                    <td class="spe_line" style="width:180px;text-align:center;cursor: pointer">
                        <span class="span_filter span_match" style="width:50px;margin:0 10px 0 10px;cursor:pointer" idvalue="{f_TeamId}">队员管理</span>
