 ...
 * Time: 21:56
 */
class RedYellowCardManageGen extends BaseManageGen implements IBaseManageGen
{
    ...
    private function GenMobileCreate(){
        ...
        if (intval($matchId) > 0) {
            ...
            $tempContent = Template::Load("league/match_event_red_yellow_card_deal_mobile.html","common");
            ...
            $arrField = $redYellowCardManageData->GetFields();
    ...
    private function GenMobileModify(){
        $resultJavaScript="";
        $redYellowCardId=Control::PostOrGetRequest("red_yellow_card_id",-1);

        $redYellowCardManageData=new RedYellowCardManageData();
        $matchManageData=new MatchManageData();
        $leagueManageData=new LeagueManageData();
        //////////////判断是否有操作权限///////////////////
        $matchId=$redYellowCardManageData->GetMatchId($redYellowCardId,true);
        $leagueId=$matchManageData->GetLeagueId($matchId,true);
        $siteId=$leagueManageData->GetSiteId($leagueId,true);
        $manageUserId = Control::GetManageUserId();
        $manageUserAuthorityManageData = new ManageUserAuthorityManageData();
        ...
        if (intval($redYellowCardId) > 0) {
            if (!empty($_POST)) {
                ...
                //记入操作log
                $operateContent = "Modify red yellow card：card：" . $redYellowCardId .",POST FORM:".implode("|",$_POST).";\r\nResult:". $result;
                self::CreateManageUserLog($operateContent);
            ...
            $oneCard=$redYellowCardManageData->GetOne($redYellowCardId);
            Template::ReplaceOne($tempContent,$oneCard);
