 ...
 * Time: 21:56
 */
class OtherEventManageGen extends BaseManageGen implements IBaseManageGen
{
    ...
    private function GenMobileCreate(){
        ...
        $otherEventManageData=new OtherEventManageData();
        if (intval($matchId) > 0) {
            if (!empty($_POST)) {
                $otherEventId = $otherEventManageData->Create($_POST,$manageUserId);
                //记入操作log
                $operateContent = "Create other event：match：" . $matchId .",POST FORM:".implode("|",$_POST).";\r\nResult:". $otherEventId;
            ...
            $tempContent = Template::Load("league/match_event_other_event_deal_mobile.html","common");
            ...
            $arrField = $otherEventManageData->GetFields();
    ...
    private function GenMobileModify(){
        $resultJavaScript="";
        $otherEventId=Control::PostOrGetRequest("other_event_id",-1);

        $otherEventManageData=new OtherEventManageData();
        $matchManageData=new MatchManageData();
        $leagueManageData=new LeagueManageData();
        //////////////判断是否有操作权限///////////////////
        $matchId=$otherEventManageData->GetMatchId($otherEventId,true);
        $leagueId=$matchManageData->GetLeagueId($matchId,true);
        $siteId=$leagueManageData->GetSiteId($leagueId,true);
        $manageUserId = Control::GetManageUserId();
        $manageUserAuthorityManageData = new ManageUserAuthorityManageData();
        ...
        if (intval($otherEventId) > 0) {
            if (!empty($_POST)) {
                $result = $otherEventManageData->Modify($_POST,$otherEventId,$manageUserId);

                //记入操作log
                $operateContent = "Modify other event：card：" . $otherEventId .",POST FORM:".implode("|",$_POST).";\r\nResult:". $result;
                self::CreateManageUserLog($operateContent);
            ...
            $oneOtherEvent=$otherEventManageData->GetOne($otherEventId);
            Template::ReplaceOne($tempContent,$oneOtherEvent);

            $state=$oneOtherEvent["State"];
