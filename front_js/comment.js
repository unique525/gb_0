...
function CreateLongComment(domain,table_id,table_type,channel_id){
    $.ajax({
        url:domain+"/default.php?mod=user&a=async_get_one",
        async: false,
        data:{},
        dataType:"jsonp",
        jsonp:"jsonpcallback",
        success:function(data){
            ...
            var name = '<span class="guest" style="text-align:left">您还未<a href="'+domain+'/default.php?mod=user&a=login&re_url="'+re_url+' style="font-weight:bold">登录</a>,目前的身份是游客</span>';
            if(result != ""){
                if(result["NickName"] != ""){
            ...
            if(tableId>0){
                $('#comment').append('<form id="mainForm" action="'+domain+'/default.php?mod=comment&a=create" method="post">'
