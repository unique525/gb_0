 ...
 * 
 * 切勿手动修改本文件！生成时间：2012/04/05 10:42:41 
 *
 * @author hightman
...
class XSSearch extends XSServer
{
	...
	public function searchForJson($query = null, $pagesize = null)
	{
		if ($pagesize = null)$pagesize=self::PAGR_SIZE;
		if ($this->_curDb !== self::LOG_DB)
			$this->_highlight = $query;
		$query = $query === null ? '' : $this->preQueryString($query);
		$page = pack('II', $this->_offset, $this->_limit > 0 ? $this->_limit : $pagesize);
	...
	public function search($query = null, $pagesize = null)
	{
		if ($pagesize = null)$pagesize=self::PAGR_SIZE;
		if ($this->_curDb !== self::LOG_DB)
			$this->_highlight = $query;
...
class XSServer extends XSComponent
{
	...
	private function connect()
	{
		...
		if (($sock = @fsockopen($host, $port, $errno, $error, 5)) === false){
                        echo $host."port:".$port."errno".$errno."error".$error;
